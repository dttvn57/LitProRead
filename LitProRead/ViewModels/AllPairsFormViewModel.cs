...
namespace LitProRead.ViewModels
{
    ...
    public class AllPairsFormViewModel
    {
        ...
        public List<SelectListItem> ProgramList { get; private set; }
        ...
        {
            this.Id = id;
            this.CurrentPair = GetPair(id);   // allPairs.Find(p => p.UniqID == id);// GetPair(id);
            if (id > 0 && this.CurrentPair != null)
            {
                EditMode = "edit";
            }
            else
            {
                RecordsCnt = GetRecordsCnt();
            }
        ...
        public PairViewModel GetPair(int id)
        {
            using (LitProReadEntities db = new LitProReadEntities())
            {
                //Tutor tutor = db.Tutors.Find(id);
                int recIndex = 0;
                PairViewModel pvm = db.GetPair(id, ref recIndex);
                if (pvm == null)
                {
                    int firstRecId = db.PairIdByRecIndex(0);
                    pvm = db.GetPair(firstRecId, ref recIndex);
                }
        ...
        public List<SelectListItem> GetProgramList(string selected = "")
        {
            using (LitProReadEntities db = new LitProReadEntities())
            {
